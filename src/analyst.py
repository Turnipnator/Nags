...
18. EXCUSED LAST-RUN OVERRIDE (added 21 May 2026 — positive mirror of Rule 2): The deterministic Form score reads finishing positions LITERALLY — it CANNOT see when a poor most-recent run was a non-recurring fluke, so it can suppress a genuinely strong horse below the NAP line. ⚠⚠ THE SOURCE MOVED — READ THIS FIRST (13 Aug 2026): this rule fires on the ★ SPORTING LIFE (HUMAN analyst) line, NOT on the "Spotlight (API, MACHINE-GENERATED)" line. The Racing API confirmed its comment is generated from the same form data you already see, and it contains a qualifying excuse in 14 of 9,619 runners (0.15%) — it essentially NEVER supplies one, and it typically reads the bad figure LITERALLY and calls it "the chief concern". Sporting Life supplies them at ~1.4% of reads. If a horse has no Sporting Life line, this rule CANNOT fire — do not infer an excuse from the generated text. Cost of getting this wrong, 13 Aug 2026: City Of Poets (Windsor 19:41 C3, scored 60.6) — Sporting Life said "Couldn't get involved having been SLOWLY AWAY and LACKED A CLEAN RUN when tenth of 14 ... dropped in grade", his form -39410 has a WIN immediately before that single poor run so guard (ii) passes, and the race is in scope. Rule 18 should have lifted him. It did not, because the rule pointed at the generated text, which said "the chief concern is his unplaced effort last time". HE WON AT 4/1. When the ★ Sporting Life read EXPLICITLY excuses the SINGLE most-recent run with a specific, race-bound reason — e.g. "drawn widest"/unfavourable draw, "badly hampered"/"met trouble"/"short of room", "never travelled on unsuitable ground", "missed the break"/"slowly away", "too keen up front", "needed the run after a long break", "race fell apart"/"wrong tactics"/"no pace to aim at", "trip too sharp on the day" — do NOT let that one run hold the horse down. Re-read its true level from its OTHER recent form + course/distance/class profile and lift adjusted_score to reflect it; the result MAY cross the 75 NAP line if the rest of the evidence supports it. STRICT GUARDS — all three required: (i) the excuse must be SPECIFIC and PRESENT in the ★ Sporting Life text, never inferred from a vague "can do better"/"remains capable" and NEVER taken from the generated API Spotlight; (ii) only the SINGLE most-recent run can be excused — if 2+ of the last few runs are poor, this rule does NOT apply even if one is excused; (iii) SCOPE: Flat Class 4+ / NH Class 3+ / Group/Listed/Grade ONLY — NEVER apply in Class 5/6, where the C5/6 calibration (Rules 14-15) and the score-vs-market gate own the compressed-pool territory and this override must not re-inflate scores. When you apply it, say so in reasoning ("last run excused: <reason>, true level is N"). Validated 21 May 2026: Bellarchi (Musselburgh 4:25, C3 hcap) — last run a Chester defeat the Spotlight excused (drawn widest) dragged deterministic Form to 9.1/22 and base to 72 (below NAP line); with that run excused her 4-time C&D record + class made her a clear 85 NAP. WON at 9/4 as part of a 3-from-3 card.
...
- EXCUSED MOST-RECENT RUN: if the ★ SPORTING LIFE (human) line gives a specific, race-bound excuse for the LAST run only (wide draw, hampered, wrong ground, missed break, too keen, needed the run), do NOT let that figure suppress the horse — re-read its true level and lift the score. See Hard Rule 18 (Flat C4+ / NH C3+ only; NEVER C5/6)
