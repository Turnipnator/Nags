...
#
# WHY: the Racing API's `comment` field is MACHINE-GENERATED from the same
# structured fields the scorer already reads (vendor-confirmed 13 Aug 2026:
# "generated by us from our own form data"), so it cannot contain the external
# insight the Spotlight override was written for. Sporting Life still carries
# human analyst copy. Proven the same day on Sudbury Hill: the API called him
# "a strong contender" off form[-1]==1, while Sporting Life revealed the C&D
# win came in a FOUR-runner race off 3lb lower with first-time cheekpieces
# that are no longer new. That killed a NAP the scorer rated 79.1.
#
# SCOPE (Paul, 13 Aug 2026): LLM judgement layer ONLY -- never the
# deterministic scorer. Text and structured `insights` only: PRICES, ratings,
# going and field size stay with the Racing API. Sporting Life's own
# `current_odds` are NOT live (Parlando showed 4/1 there against 12/1 across
# all 32 books) and must never be read.
# LIVE from 13 Aug 2026. Verified before enabling: 31/31 runners matched across
# all 5 gate-passing races on the 13 Aug card, and 0 of 2,460 deterministic
# scores moved (diffed against the pre-change scorer). Revert = one env var.
SPORTINGLIFE_ENABLED = os.getenv("SPORTINGLIFE_ENABLED", "true").lower() == "true"
SPORTINGLIFE_TIMEOUT = float(os.getenv("SPORTINGLIFE_TIMEOUT", "12"))
# Polite pacing: we only ever fetch races that already passed every gate
# (~3-8 a day, not the ~108 a full index implies). robots.txt carries no
# Disallow rules and CLAUDE.md already lists the domain as an approved source.
