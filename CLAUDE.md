...
> **✅ FOLLOW-UP FIXED 14 Aug 2026 — THE UTC DATE BUG (`src/clock.py`).** `main.py` used `date.today()`, the **CONTAINER's UTC date**. ⚠ **Correction to the first write-up: the window is 00:00–01:00 LONDON (BST), not 23:00–midnight** — UTC 23:00–23:59 *is* London 00:00–00:59 the next day, so `date.today()` returns **yesterday** and a run in that hour fetched the **wrong day's card**. Observed live at 00:42 London: `date.today()` = 13 Aug while London was 14 Aug. **⚠⚠ THE TRAP THAT MAKES A HALF-FIX WORSE THAN NO FIX: there were THREE sources of "today" and ALL THREE were UTC** — Python `date.today()`, SQLite `date('now')`, and `created_at TEXT DEFAULT CURRENT_TIMESTAMP`. Uniformly wrong, but **mutually consistent**, which is exactly why nothing had broken. **Moving only the Python side to London breaks that consistency**, and in the midnight hour `supersede_todays_selections()` and the nightly settler would silently target the **wrong day on the money ledger**. So all three moved together: new `src/clock.py` (`london_now/london_today/london_stamp`) is now the single definition; every live-path `date.today()` uses it (`main.py` ×2, `scraper.py` ×3, `analyst.py` ×2, `database.py` ×3); the two `date(created_at) = date('now')` queries (supersede + settler) now **bind the London date** instead of trusting SQLite's UTC `now`; and bot selections **stamp `created_at` on the racing clock**, so `date(created_at)` IS the racing date and SQL keeps agreeing with Python without timezone arithmetic in queries. ⚠ **CORRECTED 17 Sep 2026: that was only true for the legacy `database._save_selection` path. The LIVE path, `main._save_cherry_picks`, never passed `created_at` and kept stamping UTC for another month** — so a `/run` between 00:00 and 01:00 London (BST) would have been dated the previous day: never settled, never superseded by a later run, invisible to the Betfair bot from 01:00. **No live row ever hit it** (0 bot rows saved 23:00–01:00 UTC). Fixed: one `london_stamp()` per run on both inserts, and `log_manual_selection` now falls back to London time too (tests `tests/test_created_at_london.py` 12/12). **The Betfair bot's reader deliberately still filters on the UTC date** (Paul, 17 Sep) — the only mismatch is 00:00–01:00 London, when it shows yesterday's already-run card and catches up at 01:00, long before any race. `london_stamp()` is deliberately **naive** — SQLite's `date()`/`datetime()` cannot parse offsets. **Blast radius: outside 00:00–01:00 London this is a NO-OP on dates** (15:50 London = 14:50 UTC, same date); only the stored time-of-day string changes. Historical UTC-stamped rows are **not** rewritten. Tests **16/16** (`tests/test_clock.py`), whose load-bearing case is not "is the date right" but **"do Python, SQL and storage still AGREE"**. The scheduler was already correct (`main.py` has used `ZoneInfo(TIMEZONE)` throughout).
...
>
> **⚖ STAKE LADDER FLATTENED 17 Aug 2026 — `2.0 / 1.5 / 1.0 / 0.5` → `1.0 / 1.0 / 1.0 / 0.5`** (`config/settings.py` `STAKE_NAP` / `STAKE_NB_OF_DAY` / `STAKE_SELECTION` / `STAKE_RACE_NB` / `STAKE_DEMOTED`, read by `main.py` `_save_cherry_picks`). **LIVE, not shadow.** Full pre-flight, Paul approved ladder + live.
> **THE MEASUREMENT — 750 settled bot bets, voids/NRs/junk `race_name` rows excluded:** `nap` **n=93, −43.87pt, −14.7%, win 18.3%** · `next_best` **n=105, −31.15pt, −12.9%, win 19.0%** · `selection` n=175, −6.23pt, −2.1%, win 18.9% · `race_nb` **n=377, +12.86pt, +4.0%**, win 17.0%. **The two premium slots lost 75pt; everything else made +6.6pt.** ⭐⭐ **AND ALL FOUR SLOTS WIN AT THE SAME RATE — the NAP does not win more often than a 0.5pt cover bet, it is simply priced shorter and staked 4×.** Paired bootstrap on per-bet return: **premium minus rest −10.98%, 95% CI [−15.80, −6.43]**; **nap minus race_nb −15.65%, CI [−23.28, −7.11]. BOTH EXCLUDE ZERO** — the only cuts all week that do. Counterfactual ROI (exact, since `pnl_pts` scales linearly with `stake_pts`): full ledger −5.91% → **−3.89%**; **holdout −4.95% → −0.66%**; last 60 days −1.17% → **+3.78%**. ⭐ **Direction holds in BOTH halves** — unlike the four raw-scorer segment findings that inverted on real picks.
> **⚠ PURELY SUBTRACTIVE: it can only lower a stake, never raise one.** Total staked falls ~20%. A **fully flat** ladder (race NB 0.5→1.0) measured better still but **ADDS money at risk** and was rejected on this project's own principle; halving or deleting the premium slots measured better again but acts on n=93/105 and changes what the bot *is* rather than how it stakes. **`selection_type` is deliberately UNTOUCHED.**
> **⚠⚠ WHY THE EXCHANGE IS UNAFFECTED — verified, not assumed.** `NagsReader.load_today` selects only `race_time, race_name, horse, selection_type, odds_guide, score` — **it never reads `stake_pts`** — and sizes from its own flat constants (`BACK_FLAT_STAKE=5.0`, `PLACE_FLAT_STAKE=2.0`, `LAY_LIABILITY_CAP=5.0`). **So the exchange has been staking flat across all four slots all along.** It uses `selection_type` only for per-race priority, which is why the label must never change: altering the multiplier cannot reach live money, altering the label would. The ladder lives only in the `racing.db` ledger and on the card Paul reads for his own bets.
> **This is the 4th independent measurement to land on the NAP/NB-of-day slots** (19 Jul slot audit; #9 handicap-vs-level; the F3 review; this). ⚠ It does **not** refute "staking moves variance, not the mean" — that holds *across the same bets*; these are different slots with genuinely different per-pound returns. **Tests 16/16** (`tests/test_stake_ladder.py`), load-bearing case = **setting the env back to the old ladder reproduces pre-change output exactly**; regression suites clock/pastpost/top2/posblock/f3 all still pass. **Review 14 Sep 2026. Revert = the four env vars back to 2.0/1.5/1.0/0.5.**
>
> **⛔ F3 SHORT-PREMIUM-NAP RETIRED 17 Aug 2026 at its scheduled review** (`config/settings.py`, `FILTER_SHORTNAP_ENABLED` default true → **false**). Shadow-only throughout, so **no selection and no stake changes** — it just stops logging a filter we have now decided against. **F2 LONGSHOT stays live; F4 and F5 stay shadow (review 10 Sep).**
> **THE PREMISE DID NOT REPRODUCE — the second filter in a week to die of it.** The same cell, measured three times: **19 Jul −40.2% (n=29) → 11 Aug −39.5% (n=39) → 17 Aug +5.8% (n=40).** The 17 Aug derivation joins the ledger to the results harvest so `class`/`pattern` are **authoritative rather than keyword-guessed off `race_name`** (which never states Class 1–3), and **excludes VOID rows from the denominator** — a void returns the stake and is not a bet outcome. It is shown bet-by-bet. The 11 Aug script was ephemeral and **cannot be reconciled**, and ⚠ **that irreconcilability IS the finding: a cell whose measured ROI moves ~45 points with methodology at n≈40 cannot carry a filter.**
> **Fragility: dropping ONE bet (Indemnity, +18.75pt) takes the cell from +5.8% to −12.8%.** CI **[−49.5%, +57.0%]** — 106 points wide. By month: **+92 / +12 / +9 / −27 / −100**.
> **Its own pre-registered criterion was "promote only if the horses it flagged genuinely underperformed."** In the shadow window it flagged **11 NAPs in 4 weeks at −26.6%** — so yes, they underperformed. **But on a 27.3% strike rate against 15.5% system-wide over the same window**, and the three winners include **Pershaada 3/1 Group 3 (+7.20pt)** and Soul Love (+3.90). **Highest-strike band, losing on PRICE, demoting winners — precisely the shape F1 was retired for six days earlier.** And a DEMOTE can only cut exposure, never improve ROI: with no reliably negative cell to cut, there is nothing for it to do.
> **WIDENING (the review question — extend slot-agnostically to all premium sub-4/1, n=172): NO.** It is the only cut in the whole review whose CI excludes zero — but **only in the holdout (−30.7%) while discovery is +13.4%.** An effect that reverses sign across the split is a regime change, not an effect. **Revert = `FILTER_SHORTNAP_ENABLED=true`** (returns to shadow).
> **⚠⚠ THE REVIEW'S REAL FINDING IS THE CALENDAR, NOT THE CELL. Monthly system ROI at BOG: Mar +5.0 · Apr +3.1 · May −17.7 · Jun +10.8 · Jul +1.6 · Aug −32.9% (n=80, win 14% v ~19% baseline).** August is the worst month on record by a distance and it is where the recent bleed lives — every "deteriorating segment" found on 17 Aug (premium sub-4/1, non-handicaps, the F3 cell) is the same August showing through a different slice. ⚠ **State the confound honestly: most of this year's subtractive tightenings shipped in early August** (going-gate spatial split 5 Aug, class-floor unclassed 4 Aug, E/W place-market clamp 4 Aug, T14 min-runs 6 Aug, edge-block audit 6 Aug, Sporting Life + CHECK 19 13 Aug). **n=80 with 11 winners is comfortably inside variance — roughly four fewer winners than expected — so this is NOT evidence the changes caused it.** But it is the question worth answering next, and it is a bigger question than any single filter.
>
> **⛔ STANDING RULE 15 Aug 2026 — EVERY MANUAL CARD IS LOGGED, EVERY TIME. No asking.** Paul: *"every time we run it needs logging, or we won't know performance."* Any card where Claude **names selections with stakes** is written to `racing.db` via `database.log_manual_selection()` as **`source='manual'`**, in the same turn it is published. **NOT logged: exploratory work** — "how would you have scored that race", post-hoc winner analysis, backtests, what-ifs. Those name horses that were never bets, and logging them corrupts the exact number the 6 Aug reconciliation exists to protect. **Safe by construction:** the Betfair bot's reader filters `AND (source IS NULL OR source = 'bot')` (`/opt/betfair-bot/src/strategies/horse_racing.py:192` — **verified, not assumed**), so a manual row can never reach the exchange or stake real money; and `supersede_todays_selections()` only touches `source='bot'`, so bot and manual cards never overwrite each other. **Stake convention = the CODE's, not this file's** — NAP 2 / NB-of-day 1.5 / selection 1.0 / race NB 0.5, and `stake_pts` stores **TOTAL OUTLAY already doubled for E/W**; matching the bot is what makes bot-vs-manual comparable at all. ⚠ **Pass an explicit London `created_date`** (for backdated rows especially) — the fallback when none is passed is London time since 17 Sep 2026; before that it was SQLite's UTC `CURRENT_TIMESTAMP`. ⚠⚠ **BACKDATED ROWS MUST BE SETTLED BY HAND: the nightly settler is scoped to TODAY only** (`main.py` ~line 340, `date(created_at) = ?` bound to `london_today()`), so a row logged for any earlier date will sit unsettled forever and silently shrink the denominator. Same-day rows settle themselves. **First applied 15 Aug 2026**, backfilling 14 Aug (ids 884–889) and logging 15 Aug (ids 890–894).
> **⚠ AND THE FIRST RESULT IS A WARNING ABOUT THE MANUAL CARD, NOT AN ENDORSEMENT.** 14 Aug, same day, same rulebook: **bot −8.25pt (1 winner from 8) v Claude manual −8.50pt (0 from 6)**. The entire difference is **Cork 18:03, which the manual read PASSED and the bot backed** — Alwaysanangel, the manual card's own stated second choice, **won at 6/4 (+1.25pt)**. Both cards lost; the discretionary pass was the only thing separating them and it cost. n=1, so it proves nothing — but it is exactly why this now gets logged rather than argued about. Manual ledger all-time: **19 settled, 28.25pt staked, −0.71pt, −2.5%** (vs the authoritative bot figure ≈ −1.8% at BOG). ⚠ Also on 14 Aug: **Fast Track and Sorrengail both finished 2nd, both win-only** — correct under the E/W rule (each under 3/1) and still the recurring "our picks finish 2nd too often" pattern (key reminder #15).
>
> **Hygiene 1 Sep 2026 — `anthropic` PINNED + startup env check** (`requirements.txt`, `config/settings.py` `missing_required_env`, `main.py`, commit `9e3ac35`). The two items from the 27 Aug adversarial security review (`../IG/security_report.md`, Nags section) that survived line-by-line verification against the code. (1) `anthropic>=0.40.0` → `==0.122.0`, the version actually running in the VPS container — the Dockerfile re-installs on every rebuild, so an unpinned range pulls whatever is newest that day, and the 27 Jul ThinkingBlock incident was precisely a client behaviour change silently killing the judgement layer. Bump deliberately, never by accident. (2) `main.py` now refuses to start, naming the gap, when `TELEGRAM_TOKEN` / `TELEGRAM_CHAT_ID` / `RACING_API_USERNAME` / `RACING_API_PASSWORD` are unset — previously it started "healthy" and silently did nothing useful (chat_id 0 rejects every message; the Racing API 401s every call). `ANTHROPIC_API_KEY` deliberately NOT required (programmatic-only mode is explicit). Called from `main()` only, so scripts and tests importing settings are unaffected; VPS `.env` verified to carry all four before deploy. ⚠ **THE REST OF THAT REPORT'S NAGS SECTION IS MOSTLY WRONG FOR THIS REPO** — 13 findings checked: 4 wrong or reversed, 5 theoretical or inapplicable (Telegram uses long polling, there is no webhook; SQLite `date()` applies no timezone; the pipeline is synchronous inside one event loop so the going-snapshot "race condition" is unreachable; httpx carries a 90s timeout so nothing "hangs indefinitely"). **Two of its eight fixes would do harm:** #2 "swap all race metadata in the market swap" is a no-op at best because CHECK 0b already guarantees same-race swaps (they read the 19 Jul changelog and reported the fixed bug as current); #8 "remove commented-out speed dominance" would delete a flag-guarded revert path. **Its competitiveness-gate point is correct in premise but MEASURED AGAINST:** `_fails_competitiveness_gate`'s `len(comment) > 20` exception assumed a human Spotlight, and 62% of would-be-eliminated runners (1,637 of 2,622, 7 Jul–9 Aug) now survive on generated boilerplate — but the **5 exception-saved horses that became picks since 7 Jul returned +7.6pt (Charmaine WON 4/1, Theoryofeverything WON 9/2, Palmar Bay 3rd 25/1, two beaten).** Closing the exception would have cost money. n=5, proves nothing either way; it is memory lesson #10 again — wrong in the rubric ≠ worth fixing. Do not touch it. The one genuine code finding — **CHECK 19 computes `sl_available` across EVERY scored runner, so a NAP whose own race page failed at Sporting Life is demoted as "no SL read" while another race's fetch succeeded** — is real, small (under the flat ladder a demoted NAP only loses the double) and pre-flighted separately.
>
> **Bugfix 1 Sep 2026 — CHECK 19 demoted the NAP on a PARTIAL Sporting Life fetch** (`analyst.py` CHECK 19; tests `tests/test_sportinglife.py` §7b). `fetch_sportinglife` pulls race pages one at a time; a single page failure returns *"Sporting Life partial: N fetched, M failed"* and leaves ONE race blank while the rest are populated. CHECK 19 tested availability **card-wide** — any runner anywhere with a comment — so it read the site as "working" and demoted a NAP in the blank race as *"no Sporting Life read"*: a website hiccup counted as evidence against the horse. The 13 Aug "fails open by construction" claim was only true for a TOTAL outage. **Fix:** availability is tested against the **NAP's own race** (`_resolve_race_meta` → `runners`); if no runner in that race carries a comment, that page did not arrive and the check fails open for it, **loudly** — a `SL PARTIAL: no Sporting Life read for <course time> — NAP <horse> kept unverified` line in the compliance log. **Unchanged:** a fetched race whose NAP horse is merely unmatched still has reads on its rivals, so it still demotes; disqualifying language still demotes; a total outage is still inert; an unresolvable race falls back to the card-wide test (never guess). **Money:** re-enables a NAP in exactly one case, and under the flat ladder that is the same 1pt stake — the only added exposure is the optional 1pt double. **Method:** the partial-fetch test was written FIRST and failed 3/3 at HEAD before the fix; suite now 37/37, all gate suites unchanged. Surfaced by the 27 Aug security review — the one Nags code finding that survived verification. No new flag; `NAP_REQUIRES_SL_CORROBORATION=false` still disables the whole check.
>
> **Fix 1 Sep 2026 — RACING API RATE-LIMIT EXHAUSTION WAS SILENT** (`scraper.py` `_api_get` / `_paced_get` / `enrich_with_recent_classes`, `analyst.py` `_note_enrichment_status`, `main.py`, `config/settings.py`; tests `tests/test_api_backoff.py` 23/23). Found by the 1 Sep health check: **913 HTTP 429s between 10 Aug and 1 Sep**, in bursts of 150–244 per card (20, 21, 22, 29 Aug), enrichment taking 197–307s against a normal 0.4–32s. The horse-results endpoint is limited to **5 req/s on Pro** and the enrichment pool ran **4 unpaced workers** straight into it. Worse than the slowness: a 429 burned one of the **3 timeout attempts**, and an exhausted budget **returned None with no log line** — the runner's history was simply missing, `Enriched 227/227` counted completions not successes, and **Rule 18b and the class-drop kicker went blind for that horse with no trace anywhere**. How many histories went missing on those cards is unrecoverable. **Three fixes:** (1) **pacing** — every Racing API call is spaced process-wide to `API_RATE_LIMIT_RPS` (4.0; 0 = the old unpaced path), shared by the 4 workers under a lock; (2) **429s get their own budget** `API_429_MAX_RETRIES` (6) with 2/4/8/16s back-off, no longer consuming a timeout attempt, and **exhaustion logs a WARNING naming the endpoint**; (3) **enrichment counts** `histories / empty / FAILED` and, when FAILED > 0, sets `scraper.last_enrichment_status`, which `main.py` **prepends to the card notes** ("⚠ Racing API enrichment incomplete: N of M runner histories missing — Rule 18b and the class-drop kicker are blind for those runners"). Same principle as the Sporting Life note: a data gap goes INTO the card, never only into a log. **Money:** none when the API behaves; with pacing, more runners get the history they were designed to get, so Rule 18b fires where intended — this is the corrected behaviour, not a new signal. Failed runners still get `[]` (scorer path unchanged). ⚠ The settler's `/results` endpoint is documented at 2 req/s and makes 2 calls; unaffected. Revert: `API_RATE_LIMIT_RPS=0` restores unpaced calls; `API_429_MAX_RETRIES=2` approximates the old budget (exhaustion still logs).
>
> **✅ REVIEW CLOSED 1 Sep 2026 — T14 MIN-RUNS GUARD + EDGE-BLOCK REMOVALS: PASS, all flags stay as deployed.** Due 13 Aug, closed 19 days late (found overdue by the 1 Sep health check). Measured with `scripts/t14_edge_review.py` (now in the repo — no more throwaway scripts): every cached racecard **6–31 Aug re-scored through the live scorer with each flag toggled**, joined to results on `(race_id, horse_id)`; deterministic layer only, no Rule 18b enrichment (identical in both arms, so flag deltas are exact), no LLM. **26 days, 364 races above the class floor, 143 passing the 70/dominant-fav gates in at least one arm.** **T14 guard — trigger was 3+ suppressed horses WINNING where they would otherwise have been the selection. Result: 0 of 5.** The guard moved 36 horses' scores (all down) and changed the top betable scorer in **5 races**: Sparan Nua 7/4 (6 Aug, 4th; replacement Chicago Critic 6th), Bucanero Fuerte 7/2 (9 Aug Group 3, 2nd, race skipped), L L Koulsty 14/1 (14 Aug Listed, 3rd, race skipped), Merry 13/8 (26 Aug, 2nd; replacement 5th), Master Richard 12/1 (27 Aug, 9th; replacement 6th). **Not one won.** Two placed at prices that would have paid E/W places — state that honestly — but the trigger is wins, and the replacements lost too, so the guard cost nothing and gained nothing beyond what it was for: removing a phantom bonus. In the pre-registered 7-day window the guard changed the top scorer in 2 races, 0 winners. On the 16 real bot run days: 2 races, 0 winners. **Speed dominance — trigger was 3+ races where the bonus horse WINS and our replacement top scorer LOSES. Result: 0 of 3.** Restoring the bonus moved 40 horses and would have lifted a race past the 70 gate in **3 cases**, every one a race the live scorer correctly skipped: Percy's Lad 9/4 (18 Aug, 5th), Senecia 14/1 (22 Aug Listed, 5th), French Affair 6/5 (31 Aug, 3rd). **Not one won.** The bonus would have created three losing bets and no winning one. Restoring all three removed bonuses together (unknown-headgear +2, OR-above-field intent) moves 49 horses but changes **no additional top scorer** — those two were pure noise. **Outcome:** `T14_MIN_RUNS_ENABLED` stays true at 5, `T14_MIN_RUNS_APPLY_COLD` stays false (no new evidence either way), `SPEED_DOMINANCE_BONUS_ENABLED` / `UNKNOWN_HEADGEAR_BONUS_ENABLED` / `OR_ABOVE_FIELD_INTENT_SIGNAL` stay false. Both changes were subtractive and the window confirms the pattern: **the removed bonuses were inflating horses that then did not win.** Re-run: `.venv/bin/python scripts/t14_edge_review.py --from … --to … --run-dates …` after extending `data/harvest/`.
>
> **⛔ F4 TOP2-REDFLAG AND F5 POSITIONAL BLOCK RETIRED 11 Sep 2026 at their scheduled 10 Sep reviews** (`config/settings.py`, `FILTER_TOP2FLAG_ENABLED` and `FILTER_POSBLOCK_ENABLED` default true → **false**). Both were shadow-only throughout, so **no selection and no stake changes** — they simply stop logging. **F2 LONGSHOT stays live.** **Both failed their pre-registered bars in the direction that matters — on OUR OWN picks, flagged did BETTER than unflagged.** Measured with `scripts/pick_audit.py` on every settled primary since 13 Aug: **F5-flagged −51.6% (n=28) v unflagged −69.1% (n=20); F4-flagged −47.2% (n=15) v unflagged −63.1% (n=33).** F5 also never reached bar (a) — 34 flagged real picks against 40 required. The C+G+D ≥ 30 split that read **6/60 v 9/39 wins in Jul–16 Aug** read **1/24 v 1/14 from 17 Aug** — the effect the 13 Aug "CI spans zero" caveat warned about did not hold forward. That makes F5 the **fifth raw-scorer segment finding to invert on real picks** (after handicaps #9, few-run #10, tied-block #11, non-handicap arm #13) — the raw scorer's segments do not describe the card the judgement layer actually produces. F4's discovery effect (flagged top scorer wins 6% v 15%) never reached the card because the judgement layer already ducks those spots; the 10 Aug holdout inversion repeated. Revert = the flag back to `true` (returns to shadow, since both `*_SHADOW` flags still default true). ⚠ **THE FINDING THAT MATTERS FROM THE SAME AUDIT IS NOT ABOUT EITHER FILTER — see `project_pick_vs_danger_sep2026`:** since 17 Aug the primary slot is **2 winners from 38** while the bot's **own named DANGER won 9** and finished ahead of the pick in **22 of 35**; the pattern begins in July (danger 21 v pick 15, n=99) and was absent Apr–Jun (49 v 52, n=244). The 30 Jun scorer recalibration is **ruled out** (`scripts/recal_ab.py`: live top scorer −23% v old −35% at SP over 373 gate-passing races; where they disagree live won 15 v 8 — do NOT revert `SCORER_RECAL_ENABLED`). Rule 18b lifts, LLM lifts and score construction all checked and clean. **Cause unidentified. Do not write a "back the danger" rule off this.**
>
> **Tightening 11 Sep 2026 — CHECK 21: SCORE FLOOR ON EVERY SLOT** (`analyst.py` CHECK 21 + shared `_drop_primaries` helper, `config/settings.py` `SCORE_FLOOR_ALL_SLOTS`, tests `tests/test_score_floor.py` 23/23). Full pre-flight, Paul approved with all defaults. **CHECK 13 floored the NB-of-day slot only.** On 8 Sep a **61 (Spring Bloom) was saved as a full 1pt E/W selection** because the market swap (CHECK 1) fired between a 63 and a 61 and neither the swap nor the selection slot has a floor; on 11 Sep a **41 (Nightime Dancer) went in as a race NB**. Operating Policy says below 70 is not a selection and below 55 is a pass; the staking layer now enforces it: **primary 55–69 → the same demote CHECK 13 uses** (`nb_price_capped` → `STAKE_DEMOTED` 0.75pt, E/W forced where a place market exists); **primary < 55 → DROPPED** through the same helper F2 uses (`nap_index` remapped, double rebuilt); **race NB < 55 → dropped. Race NBs at 55–64 deliberately untouched** — they returned **+15.8% (4 winners from 13, 28 Jul–10 Sep)**, so the race-NB floor is 55, not 65. Missing score fails OPEN; a NAP is 75+ by construction so it is never touched. **Cost-checked before shipping** (lesson #10): since scores were first stored for every slot on 28 Jul it would have touched **four settled bets** — two primaries at 62 and 66 (−1.25pt on 3.5 staked) and two race NBs at 43 and 54 (both beaten) — about **+1.5pt in six weeks. This is hygiene, not edge**; its value is that a swap between two sub-70 horses can no longer defeat the ladder. **Money:** strictly subtractive on the ledger; on the exchange it can only REMOVE a place bet (a dropped row no longer exists — `nags_place` keys on the row), never add one. A demoted 55–69 primary keeps its `selection_type`, so the exchange still stakes its flat £2 on it, exactly as CHECK 13 demotes do today. **Method:** the test was written first and **failed 9 of 23 at HEAD** before the gate existed; F2's drop-and-remap was factored into `_drop_primaries` and its own test still passes byte-identically. Also fixed while there: the Telegram **staking text had been hard-coded to the pre-17-Aug 2.0/1.5/0.75 ladder for four weeks** after the flattening; it now reads the live `STAKE_*` constants. **⏰ REVIEW 25 Sep 2026** (Paul: *"we can revert back after a couple of weeks if it doesn't improve"*). Revert: `SCORE_FLOOR_ALL_SLOTS=false`, one env var. What to look at: every `SCORE FLOOR:` compliance line and what the demoted/dropped horse did.
>
> **Tightening 17 Sep 2026 — ODDS-ON FAVOURITE PASS + WRITE-UP RULES** (`analyst.py` `_oddson_fav_small_field` in both race-ranking branches, prompt section *WRITE-UP RULES*, CHECK 16 *F2 CONSENSUS* shadow line, `scraper.py` `Runner.odds_median`, `config/settings.py`; tests `tests/test_oddson_fav_pass.py` 36/36). Full pre-flight, Paul approved *"yes to all, go live but evens is ok, just odds on"*. **Triggered by Southwell 17:25 (C4 2m hcap, 4 runners):** Level Look was **4/9**, so the sub-evens block kept him off the card and the bot backed the leftovers — **Charging Thunder 10/1** (8yo, Flat form 502700, 14th of 15 beaten 57¼L last time, mark cut 95→84 since March) as the NB-of-day slot and **New York Minute 10/1** as race NB. **No existing gate could see it:** the betable-70 gate passed on Charging Thunder's 73.2, and the dominant-favourite pass rule needs the favourite 8 RPR clear, while Level Look had the **lowest** RPR (the ability anchor also sank him to last on 44.5 — it ranks on raw RPR/TS/OR with **no allowance for his 6lb penalty or weight-for-age**; real, but NOT fixed here because it would not have changed the pick and it moves scores). The write-up read *"Clear figures edge in a 4-runner heat: RPR 96 (+12 over OR 84)"* — the recent form, the 4/9 favourite and the Sporting Life read all went unmentioned. **The two rows (ids 1030/1031) were superseded by hand at 11:44 London before any bet**; the Betfair bot reloaded 4 picks at 11:45.
> **(1) ODDS-ON FAVOURITE PASS — LIVE.** Favourite **strictly shorter than evens** AND **≤ `ODDSON_FAV_MAX_FIELD` (6) runners** ⇒ race dropped before judgement, logged `→ DROPPED-oddsonfav:`. **Evens does NOT fire** (Paul's call) — deliberately looser than the sub-evens selection block (≤1/1). Field = every runner after the NR filter, so a runner unpriced at Bet365 still counts; no parseable price ⇒ does not fire. **Evidence, 774 settled bot bets joined to the racecard cache:** odds-on favourite + ≤6 runners = **51 bets / 30 races, 4 wins, −40.8% ROI (−29pt on 71pt)**; **−27.1% to 30 Jun (n=38), −82.9% since (n=13)** — same direction in both halves. ⚠ **State the weakness:** per-bet CI **[−85%, +37%] spans zero**; the 6-runner cut was chosen **after** seeing this race; favourite prices in the cache are **near-off, not morning**, so the live rule (Bet365 at run time) fires on a somewhat different set. Fields of 7+ went the other way (**+20.2%, n=41**) and "any field size" **flips sign across the halves (+3.0% → −87.5%)**, so neither is used. **Money:** it removes a race, but a dropped race can let the next qualifying race into judgement, so it can swap a bet rather than only remove one. The exchange needs no change — a dropped race writes no rows. **Replay of the 17 Sep live card: old code sent Ayr 15:00 / Southwell 17:25 / Pontefract 15:08; new code drops only Southwell 17:25.** **⏰ REVIEW 15 Oct 2026. Revert trigger: 3 passed races where our would-be pick WINS. Revert: `ODDSON_FAV_PASS_ENABLED=false`.**
> **(2) WRITE-UP RULES (prompt).** On a no-NAP card Telegram shows only `reasoning[0]`, so it must now carry the case for AND against; the market favourite must be named with its price when opposed; recent form must be stated in plain words and a rating overlay never cited without saying whether recent runs support it; the ★ Sporting Life read must be used, and the machine-generated API comment never presented as the case; no raw field dumps (`C&D: True`) and no quoting a score different from the one output. ⚠ Judgement-layer prose — **not gate-enforceable and not separately measurable**, and it can also shift selections. The card now also prints **Danger** under every pick not already shown in the NAP/NB blocks.
> **(3) F2 CONSENSUS — SHADOW, log only.** Live F2 reads Bet365; Charging Thunder was 10/1 there while **16 of 29 books had him 11/1+**. New `Runner.odds_median` (median decimal across books − 1, same fractional units as F2) and a `FILTER-SHADOW F2 CONSENSUS:` line whenever F2 on the median would decide differently from F2 on Bet365, in both directions. Mutates nothing. Flag `F2_CONSENSUS_SHADOW_ENABLED`. Judge at the 15 Oct review.
> **(4) Stake wording.** The no-NAP header and staking lines read the live ladder and show the demoted 0.75pt stake; CHECK 12's note no longer quotes the retired 1.5pt NB-of-day stake.
> ✅ **FIXED SAME DAY — the 16-of-39 `field_size` mismatches were withdrawn horses still being scored.** See "Bugfix 17 Sep 2026 — NR saddle-cloth flag" below.
>
> **Bugfix 17 Sep 2026 — NR SADDLE-CLOTH FLAG: withdrawn horses had been scored since 17 Aug** (`scraper.py` `_parse_race` + `_is_nr_flagged`, `config/settings.py` `NR_NUMBER_FLAG_ENABLED`; tests `tests/test_nr_flag.py` 16/16). **The 9 Jul non-runner filter silently stopped working on 17 Aug 2026.** It treats a runner as withdrawn only when NO bookmaker quotes it — true on 9 Jul (0/40 mismatches). **From 17 Aug the Racing API keeps stale bookmaker prices on withdrawn horses:** 0–4 a day still priced before that date, then essentially all of them (Aug 558/1,017, Sep 369/393); Bet365 itself still quoted 12 of the 24 on the 17 Sep card. So for a month withdrawn horses were scored, counted toward field size (NB-of-day 8+, E/W place market 5+, system-resistant 12+, the new ≤6 odds-on pass), left in every field-relative comparison (top-RPR-in-field, speed ranks, the C4-and-below ability anchor, market-favourite detection), and were selectable. **On the 17 Sep 10:05 run 16 of 39 races carried them (24 horses); 20 of the 65 races the bot bet in since 17 Aug were scored with at least one** (from the run-time warnings — a floor, since a few LLM race names do not match the log). **The API's real withdrawal marker is `number == "NR"`** in place of the saddle-cloth number: **5,349 NR-flagged runners Apr–Sep, 0 appear in any result.** Reserves carry an R-number (`R17`) and **do** run (68/68) — only the exact token `NR` is dropped. Fix: a flagged runner is dropped first whatever its prices; the existing price rule then runs over the remaining runners as a backstop, so a stale price on a withdrawn horse cannot make an unopened market look open. **Blast radius, honestly stated:** re-scoring the cached cards 17 Aug–12 Sep (fetched after racing, so an UPPER bound — includes horses withdrawn after the morning run): 545 races changed runner count, real runners' scores moved in 304, 14 races flipped the 70+ gate (4 in, 10 out), and in 13 gate-passing races the top betable scorer was a withdrawn horse. **No evidence it cost winners:** only 4 races had a withdrawn horse change which RUNNING horse topped the race, and in all 4 the contaminated ranking's top horse finished better. Of the 5 bot picks since 17 Aug that became non-runners, **Pure Mint (31 Aug NAP), Spring Bloom and Faiyum were withdrawn AFTER the run** (no run-time mismatch in their races); Cranachan and Breizh River are ambiguous. ⚠ **The onset coincides with the 17 Aug start of the primary-slot slump (2 winners from 43) — but that window begins on 17 Aug because of the stake-ladder change, and nothing measured here shows the ghost runners caused it. Do not cite this fix as the explanation.** **Verification:** flag off ⇒ runner lists identical to the previous code in all **6,575** cached races; flag on ⇒ mismatches since 17 Aug **547 → 2** (both one unpriced horse, "Damascus Steel I", who ran — a pre-existing price-rule false positive, not touched); live replay of the 17 Sep card: mismatch warnings **16 → 0**, runners scored 444 → 421, **same races to judgement with the same top horses.** This is a data-correctness fix, not a strategy bet — live, no paper trade; check at the 15 Oct review. Revert: `NR_NUMBER_FLAG_ENABLED=false`.
>
> **Logging 17 Sep 2026 — SPORTING LIFE FLAG LOG (`analyst.py` `_log_sl_flags`, `config/settings.py` `SL_FLAG_LOG_ENABLED` / `SL_FLAG_LOG_PATH`; tests `tests/test_sl_flag_log.py` 17/17). LOG ONLY — mutates nothing.** Paul asked for it after Ayr 15:00: **Wild Thoughts**, a 3yo Sporting Life flagged `AHEAD_OF_THE_HANDICAPPER`, scored **53.1 (10th of 11)** and **WON at 11/4F**, while our two top scorers — Financer 75.3 and Altareq 66.8, both C&D winners banking the full 37 course/going/distance points while scoring 4 and 6 of 12 on class — finished **8th and 11th**. The same morning the other flagged favourite, Level Look (4/9), was also a 3yo the scorer put last (44.5). The scorer gives an unexposed horse the neutral 18.5 for course/going/distance, so it cannot see this case; **and Sporting Life pages are not cached, so there is NO history — this log is the only record the flag will ever have.** Writes `/app/data/sl_flag_log.jsonl` (the `./data` host volume — survives rebuilds, ⚠ **not** in the nightly DB backup): one line per gate-passing race per run, with **every** runner's deterministic score, rank in race, Bet365 and median price, whether a Sporting Life read arrived (`sl_read`, so "no read" is never mistaken for "not flagged"), its SL insight flags, and the `sl_status` of the fetch. Unflagged runners are the comparison group. A readable `SL-FLAG AHEAD_OF_THE_HANDICAPPER:` line per flagged horse also goes to `bot.log`. Any write failure logs a WARNING and never touches the card; races to judgement are identical with the log on or off (tested). **Join results later on date + course + race time + horse; a later run the same day repeats the race, so keep the last `logged_at` per race.** ⚠ **No rule off this before it is measured** — the additive-edge trap and F5's forward inversion both apply. **Pre-registered question:** do flagged runners the scorer ranks outside its top 2 beat their price (A−E v unflagged runners in the same races)? **Look at ~15 Oct with the other reviews; act only with ≥50 flagged runners and a CI that excludes zero, and even then shadow first.** Revert: `SL_FLAG_LOG_ENABLED=false`.
>
> **Bugfix 18 Sep 2026 — FIELD_SIZE RECONCILIATION + CHECK 22 (NO-PRICE SELECTIONS)** (`scraper.py` `_parse_race`/`_parse_runner`, `analyst.py` CHECK 22, `config/settings.py` `FIELD_SIZE_RECONCILE_ENABLED` / `NO_PRICE_SELECTION_DROP`; tests `tests/test_field_size_reconcile.py` 14/14 + `tests/test_no_price_guard.py` 16/16). Full pre-flight, Paul approved reconcile + guard. **An 08:09 `/run` — earlier than usual, before the market opened — exposed BOTH pre-NR-flag heuristics at once.** **Ayr 15:40 Bronze Cup: 24 runners, `field_size` 24, only 9 PRICED** — the 9 Jul price rule ("unpriced while a rival is priced = withdrawn") dropped 15 real runners and **the race reached judgement scored as a 9-runner handicap** (every field-relative score, the 12+ system-resistant threshold and the E/W terms wrong; the three `AHEAD_OF_THE_HANDICAPPER` flags logged that morning carry that wrong field — exclude that line at review). **Newbury 17:17: 30 runners, `field_size` 30, NOTHING priced, 16 jockeys undeclared** — the 4 Aug-narrowed jockey rule dropped those 16. Nothing was withdrawn in either race; the market simply had not opened. **No pick came from either race — luck, not a gate.**
> **(1) RECONCILIATION.** `field_size` is the API's own count of actual runners, so it now decides HOW MANY; the `NR` saddle-cloth flag (17 Sep) names WHICH. Unpriced runners are dropped **only** when the card still holds more runners than `field_size` AND they **exactly** account for that surplus — otherwise we cannot tell which one is the non-runner, so all are kept and the mismatch warning fires. The jockey rule is folded in: a jockey-less runner is simply an unpriced runner for counting. ⚠ **`field_size` 0 or missing NEVER empties a race** (caught by the 4 Aug suite: a fixture with `field_size` 0 would have wiped the field). **Verified on 6,575 cached races: 39 races change, every one RESTORING runners, ZERO runners removed anywhere; mismatches 185 → 146** (the residual is the known Irish self-inconsistency plus 43 races where the API's own count exceeds its own card). ⚠ **The pre-flight predicted 7 changed races and was WRONG — it measured raw cards rather than the old code's output.** The 39 include whole early-harvested meetings (Brighton 18 Apr, Epsom 21 Apr, Plumpton 10 May, Musselburgh 21 Jul, Fairyhouse 5 Apr) where the old code kept as few as **ZERO** runners. **The 3 genuine catches still fire** (1 Jun: card 9 v `field_size` 7, exactly 2 unpriced). Flag off ⇒ output identical to the previous code across all 6,575 races. Live replay: Newbury 17:17 14 → 30, warnings 1 → 0, same races to judgement (Ayr 15:40 had self-healed by then — the market had opened, which is exactly why this only bites on an early run). **Revert: `FIELD_SIZE_RECONCILE_ENABLED=false`.**
> **(2) CHECK 22 — a selection with NO PRICE is dropped.** Reconciliation deliberately KEEPS unpriced runners in the field (they must count), so the judgement layer now sees more of them — and **nothing had ever stopped it picking one**: 8 rows exist with `odds_guide = "CHECK PRICE"`, including a **4pt NAP (Fillyoureye, 4 Apr, went off 3/1F, lost)** and a next-best that started **4/6** (Minnie Hauk, 4 May, won). **12.5pt staked, −7.6pt returned.** Worse, an unpriced pick is invisible to every price gate — sub-evens block, F2 longshot and the 10/1 NAP cap all read `_parse_odds_to_decimal` as 0 and wave it through. CHECK 22 drops such a primary through the same `_drop_primaries` helper F2 and CHECK 21 use (`nap_index` remapped, double rebuilt) and clears an unpriced race NB. **Strictly subtractive; on the record above it would have saved 7.6pt in six months.** Prices that merely look odd (Evens, 4/6, 100/1) are untouched — other gates own those. **Revert: `NO_PRICE_SELECTION_DROP=false`.**
> ⚠ **Method note: an early `/run` is when these surface.** Both heuristics predate the `NR` flag and were only ever guesses at withdrawal; the flag has been authoritative since 17 Sep (0 of 5,349 flagged runners appear in a result). What is NOT addressed: whether a card should be scored at all when the market has barely opened — Ayr 15:40 had 15 of 24 unpriced at 08:09, so its prices, and every odds-keyed gate, were half-formed. That is a separate question and deliberately not answered here.
>
...
## Pass-the-Race Rule — Odds-On Favourite in a Small Field (added 17 Sep 2026)

**Principle (manual AND bot):** when the market favourite is **odds-on (shorter than evens)** in a field of **6 or fewer**, **PASS the race.** The sub-evens block rightly keeps the favourite off the card, which leaves only the horses the market rates least likely, usually with no or poor place terms. An **Evens** favourite does NOT trigger this.

**Distinct from both earlier pass rules.** The betable-threshold gate keys on our scores; the blocked-dominant-favourite rule keys on the favourite's RPR lead. This one keys on the **market** — it fires even when the favourite has the lowest figures in the race, which is exactly the case the other two cannot see.

**Triggered by Southwell 17:25, 17 Sep 2026:** Level Look 4/9 in a 4-runner Class 4 handicap. The bot backed Charging Thunder 10/1 and New York Minute 10/1. Both rows were superseded by hand before any bet was struck.

**Evidence:** 51 bot bets in 30 such races, 4 wins, −40.8% ROI, negative in both halves (−27.1% / −82.9%). CI spans zero and the 6-runner cut was chosen after the event — this is a subtractive bet on a consistent direction, not a proven effect.

**Implementation:** `analyst.py` `_oddson_fav_small_field(race_scored)`, flags `ODDSON_FAV_PASS_ENABLED` / `ODDSON_FAV_MAX_FIELD` (6). Applied at race-selection in BOTH `/run N` and default modes, after the dominant-favourite rule. Logs `→ DROPPED-oddsonfav:` lines.

**Revert path:** review 15 Oct 2026. If 3 passed races are won by the horse we would have backed, set `ODDSON_FAV_PASS_ENABLED=false`. Do not widen to 7+ runners (measured +20.2%).
...
SINGLES (80%+ of total outlay):
  NAP of the Day:           1 pt  (Win or E/W per table)   ← was 2 pts
  Next Best (across card):  1 pt  (Win or E/W per table)   ← was 1.5 pts
  Race selections:          1 pt each
  Race NB picks:            0.5 pts each

  ⚠ FLATTENED 17 Aug 2026. The NAP and NB-of-day slots lost 75pt all-time
  while race selections + race NBs made +6.6pt — and ALL FOUR SLOTS WIN AT
  THE SAME RATE (18.3 / 19.0 / 18.9 / 17.0). The NAP does not win more often
  than a 0.5pt cover bet; it is simply priced shorter and was staked 4x.
  See the header note "STAKE LADDER FLATTENED" for the full measurement.
  The NAP is still called, still capped at 10/1, still needs 75+ and Sporting
  Life corroboration — only the stake multiple changed.
