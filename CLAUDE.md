...
> **⚠ OPEN DECISION — `NAP_REQUIRES_SL_CORROBORATION` (built, default FALSE, changes nothing).** Proposed so that "the case is stronger when they both join up" cashes out **subtractively**: a NAP would REQUIRE Sporting Life support, making the NAP harder to earn rather than making any stake bigger. It lands on the NAP slot, which three independent measurements identify as where we bleed, and **Sudbury Hill would have failed it on 13 Aug**. Not enabled — awaiting an explicit decision.
>
